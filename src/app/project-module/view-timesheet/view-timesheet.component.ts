...
export interface WeeklyPeriodicElement {
  timesheetId: string;
  projectName: String;
  timesheetStatus: String;
  startDate: number;
...
const DAILY_ELEMENT_DATA: DailyPeriodicElement[] = [
  {
    timesheetId: '1',
    projectName: 'Citi',
    timesheetStatus: 'Approved',
    dateCreated: 2023 - 2 - 20,
    hours: 1,
    timesheetType: 'Daily',
  },
  {
...
const WEEKLY_ELEMENT_DATA: WeeklyPeriodicElement[] = [
  {
    timesheetId: '1',
    projectName: 'Citi',
    timesheetStatus: 'Approved',
...
export class ViewTimesheetComponent {
  ...
  totalHours: number = DAILY_ELEMENT_DATA.map((el) => el.hours).reduce(
    (a, b) => a + b,
    0
  );
  dataSource?: any = new MatTableDataSource(DAILY_ELEMENT_DATA);
  ...
  applyFilterForCategory() {
    ...
    if (this.selectedValue === 'All') {
      ...
      this.dataSource = new MatTableDataSource(DAILY_ELEMENT_DATA);
      this.totalHours = DAILY_ELEMENT_DATA.map((el) => el.hours).reduce(
        (a, b) => a + b,
        0
      );
    } else {
      ...
      if (this.selectedValue === 'Daily') {
        ...
        this.columnShown = this.dailyDisplayedColumns;
        this.dataSource = new MatTableDataSource(DAILY_ELEMENT_DATA);
        this.totalHours = DAILY_ELEMENT_DATA.filter(
          (el) => el.timesheetType === 'Daily'
        )
          .map((el) => el.hours)
          .reduce((a, b) => a + b, 0);
        ...
        this.dataSource.filter = this.selectedValue;
        this.totalHours = WEEKLY_ELEMENT_DATA.filter(
          (el) => el.timesheetStatus === 'Approved'
        )
          .map((el) => el.hours)
          .reduce((a, b) => a + b, 0);
      } else {
